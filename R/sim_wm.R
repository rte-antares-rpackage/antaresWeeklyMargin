...
#' @param start_prev_hebdo Date of forecasts.
...
#' 
#' }
sim_wm <- function(date_prev, start_prev_hebdo, path_inputs = path_sim_wm(), opts = antaresRead::simOptions()) {
  ...
  # Add CNES forecast
  opts <- create_wm_load_fr(path = path_inputs$cnes, date_prev, "prevu")
  ...
  opts <- create_wm_ror(data = oa, start= date_prev,  startday = "samedi", opts = opts)
  opts <- create_wm_misc(data = oa, start = start_prev_hebdo, opts = opts)
  
  # NTC
  ntc <- fread(file = path_inputs$ntc)
  opts <- create_wm_ntc(data = ntc, start = start_prev_hebdo, opts = opts, startday = "samedi")
